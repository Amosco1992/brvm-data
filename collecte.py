...
# ==========================================================================
# ÉTATS FINANCIERS — extraction pluriannuelle
# ==========================================================================

# Multiplicateurs déclarés dans les en-têtes de section.
# Les émetteurs publient indifféremment en français ou en anglais — Sonatel
# dépose une version « (in million XOF) » là où un industriel ivoirien écrit
# « (Montants en milliers de FCFA) ». Les deux doivent être reconnues, sans
# quoi l'extraction rend zéro sans rien signaler d'anormal.
ECHELLES = [
    (r"(?:en|in)\s+milliards?\s+(?:de\s+)?", 1_000_000_000),
    (r"(?:en|in)\s+billions?\s+(?:of\s+)?", 1_000_000_000),
    (r"(?:en|in)\s+millions?\s+(?:de\s+|of\s+)?", 1_000_000),
    (r"(?:en|in)\s+milliers?\s+(?:de\s+)?", 1_000),
    (r"(?:en|in)\s+thousands?\s+(?:of\s+)?", 1_000),
]
DEVISES = [
...
POSTES = [
    # Le résultat part du groupe d'abord : c'est lui qui revient aux
    # actionnaires. Le résultat consolidé inclut les minoritaires et
    # gonflerait artificiellement le bénéfice par action.
    ("resultat_part_groupe", [
        r"r[ée]sultat\s+net(?:,)?\s+part\s+du\s+groupe",
        r"net\s+income\s+attributable\s+to\s+the\s+owners",
        r"net\s+income\s+attributable\s+to\s+owners",
        r"r[ée]sultat\s+net\s+attribuable\s+aux\s+propri[ée]taires",
    ]),
    ("resultat_net", [
        r"r[ée]sultat\s+net\s+consolid[ée]",
        r"consolidated\s+net\s+(?:income|profit|result)",
        r"r[ée]sultat\s+net\s+de\s+l.exercice",
        r"net\s+profit\s+for\s+the\s+(?:year|period)",
        r"r[ée]sultat\s+net(?!\s+par\s+action)(?!\s*,\s*part)(?!\s+attribuable)",
        r"b[ée]n[ée]fice\s+net",
    ]),
    ("produit_exploitation", [
        r"produit\s+net\s+bancaire",
        r"net\s+banking\s+income",
...
# Le bénéfice par action et la résolution de dividende, quand l'émetteur les
# publie : ce sont les deux chiffres dont dépend le taux de distribution, et
# ils évitent de le reconstituer à partir d'un nombre d'actions estimé.
POSTES_TOUS = None  # complété plus bas, une fois POSTES_SUP défini

MOTIFS_BPA = [
    r"r[ée]sultat\s+net\s+par\s+action[^\n]*?\bbasic|net\s+income\s+per\s+share",
    r"b[ée]n[ée]fice\s+par\s+action",
]
MOTIF_DIV_BRUT = re.compile(
    r"(?:dividende\s+brut[^\n]{0,60}?|gross\s+dividend[^\n]{0,60}?)"
    r"([\d][\d\s.,]{2,12})\s*(?:XOF|FCFA|F\s?CFA)", re.I)
MOTIF_DIV_NET = re.compile(
    r"(?:dividende\s+net[^\n]{0,60}?|net\s+dividend[^\n]{0,60}?)"
    r"([\d][\d\s.,]{2,12})\s*(?:XOF|FCFA|F\s?CFA)", re.I)
MOTIF_IRVM = re.compile(r"(\d{1,2})\s*%\s*(?:de\s+)?(?:retenue|withholding)[^\n]{0,30}?irvm"
                        r"|irvm[^\n]{0,30}?(\d{1,2})\s*%", re.I)
...
# Deux écritures cohabitent selon l'extracteur PDF : « 1 923 122 » avec
# séparateurs, et « 1923122 » quand ils ont sauté. La première alternative
# capte les nombres groupés — en exigeant que le dernier groupe ne soit pas
# suivi d'un chiffre, sinon « 1923122 1776443 » se lirait « 1923122 177 ».
# La seconde capte les suites brutes.
# Trois écritures cohabitent : « 1 399 263 » (espaces), « 1.399.263 » (points,
# fréquent dans les attestations de CAC) et « 1399263 » quand l'extraction PDF
# a mangé les séparateurs. Les formes groupées d'abord, la suite brute ensuite.
_GROUPE = re.compile(
    r"\(?-?(?:\d{1,3}(?:,\d{3})+(?!\d)"
    r"|\d{1,3}(?:\.\d{3})+(?!\d)"
    r"|\d{1,3}(?:[\s\u00a0\u202f]\d{3})+(?!\d)"
...
def _valeur_fin(txt: str) -> float | None:
    ...
    t = re.sub(r"[()\s\u00a0\u202f]", "", txt)
    # « 1.399.263 » : des points tous les trois chiffres sont des séparateurs
    # de milliers, pas une virgule décimale. Le confondre divise la valeur
    # par mille et passe inaperçu.
    if re.fullmatch(r"-?\d{1,3}(?:\.\d{3})+", t):
        t = t.replace(".", "")
    # En français la virgule est décimale — « 266,44625 FCFA » vaut 266,4 et
    # non 26 millions. Elle ne sépare des milliers que dans la forme anglaise
    # « 1,399,263 », reconnaissable à ses groupes de exactement trois chiffres.
    if re.fullmatch(r"-?\d{1,3}(?:,\d{3})+", t):
        t = t.replace(",", "")
    else:
        t = t.replace(",", ".")
...
def extraire_etats(texte: str, devise_attendue: str = "XOF") -> dict:
    ...
    for brute in texte.splitlines():
        ...
        if re.search(r"montants?\s+en|exprim[ée]s?\s+en|amounts?\s+in|\((?:en|in)\s+",
                     ligne, re.I):
        ...
        for poste, motifs in POSTES:
            # Les intitulés verticaux du gabarit (« BALANCE SHEET ») sont
            # extraits lettre par lettre et se collent en tête de ligne :
            # « T Total owner's equity ... ». On tolère ce parasite.
            prefixe = r"^[\s\-–—•]*(?:[A-Z]\s+)?"
            motif = next((m for m in motifs if re.search(prefixe + m, bas, re.I)), None)
            if not motif:
                continue
            reste = ligne[re.search(prefixe + motif, bas, re.I).end():]
...
# ---------------------------------------------------------------------------
# Analyse fondamentale : séries pluriannuelles
# ---------------------------------------------------------------------------
#
# Un exercice isolé ne dit presque rien. Une marge de 12 % est excellente si
# elle était de 8 % il y a cinq ans, inquiétante si elle était de 20 %. Tout
# ce qui suit sert à transformer des instantanés en trajectoires.

POSTES_SUP = [
    ("dettes_financieres", [
        r"total\s+(?:des\s+)?dettes\s+financi[èe]res",
        r"emprunts?\s+et\s+dettes\s+financi[èe]res",
        r"(?:total\s+)?(?:non-current\s+)?financial\s+liabilit(?:y|ies)",
        r"dettes\s+financi[èe]res\s+non\s+courantes",
    ]),
    ("tresorerie", [
        r"tr[ée]sorerie\s+et\s+[ée]quivalents?",
        r"cash\s+and\s+cash\s+equivalents",
        r"disponibilit[ée]s",
    ]),
    ("flux_exploitation", [
        r"(?:flux|cash)\s+(?:net\s+)?(?:de\s+)?tr[ée]sorerie\s+(?:g[ée]n[ée]r[ée]s?\s+par\s+)?"
        r"l.(?:activit[ée]|exploitation)",
        r"cash\s+generated\s+from\s+operating\s+activities",
        r"flux\s+net\s+de\s+tr[ée]sorerie\s+d.exploitation",
    ]),
]


def exercice_du_document(url: str, texte: str) -> int | None:
    """L'exercice auquel se rapporte un document.

    Piège : la date de publication figure toujours en tête et vaut souvent
    l'année suivante — les comptes 2025 de Sonatel sont publiés en février
    2026. Prendre l'année la plus récente vue en première page rattache donc
    systématiquement le document au mauvais exercice. On cherche d'abord une
    date de clôture explicite, dans tout le document.
    """
    m = re.search(r"exercice[_\s-]*(\d{4})|exo[_\s-]*(\d{4})|fy[_\s-]*(\d{4})", url, re.I)
    if m:
        return int(next(g for g in m.groups() if g))

    cloture = re.search(
        r"31\s*(?:/|-|\s)?\s*(?:12|d[ée]cembre)\s*(?:/|-|,)?\s*(\d{4})"
        r"|december\s+31\s*,?\s*(\d{4})", texte, re.I)
    if cloture:
        return int(cloture.group(1) or cloture.group(2))

    # Dernier recours : l'année la plus citée, et non la plus récente.
    annees = re.findall(r"\b(20[0-3]\d)\b", texte[:6000])
    if annees:
        return int(max(set(annees), key=annees.count))
    return None


def tous_etats_financiers(session, url_societe: str, timeout: int = 30,
                          maxi: int = 8) -> list[dict]:
    """Tous les PDF d'états financiers d'un émetteur, du plus récent au plus ancien."""
    try:
        r = session.get(f"{url_societe}?field_type_rapport_tid={TID_ETATS}",
                        headers=ENTETES_PDF, timeout=timeout)
        r.raise_for_status()
        ...
        return []
    vus, docs = set(), []
    for lien in re.findall(r'href="([^"]+\.pdf)"', r.text, re.I):
        url = RACINE_SITE + lien if lien.startswith("/") else lien
        if url in vus:
            continue
        vus.add(url)
        m = re.search(r"/(\d{8})_", url)
...
def serie_fondamentale(exercices: dict[int, dict]) -> dict:
    """Trajectoires et régularité, à partir des exercices extraits.

    On ne calcule une tendance qu'à partir de trois exercices : sur deux
    points, toute variation ressemble à une tendance.
    """
    annees = sorted(e for e, p in exercices.items()
                    if (p.get("resultat_net") or {}).get("valeur"))
    if len(annees) < 2:
        return {"exercices": len(annees), "suffisant": False}

    def val(a, poste):
        return (exercices[a].get(poste) or {}).get("valeur")

    def croissance(poste):
        pts = [(a, val(a, poste)) for a in annees if val(a, poste)]
        if len(pts) < 3:
            return None
        (a0, v0), (a1, v1) = pts[0], pts[-1]
        if v0 <= 0 or a1 == a0:
            return None
        return round((pow(v1 / v0, 1 / (a1 - a0)) - 1) * 100, 1)

    marges = {a: round(val(a, "resultat_net") / val(a, "produit_exploitation") * 100, 1)
              for a in annees
              if val(a, "produit_exploitation") and val(a, "resultat_net")}
    roes = {a: round(val(a, "resultat_net") / val(a, "capitaux_propres") * 100, 1)
            for a in annees if val(a, "capitaux_propres") and val(a, "resultat_net")}

    resultats = [val(a, "resultat_net") for a in annees]
    pertes = sum(1 for v in resultats if v < 0)
    reculs = sum(1 for i in range(1, len(resultats)) if resultats[i] < resultats[i - 1])

    dernier = annees[-1]
    dettes, cp = val(dernier, "dettes_financieres"), val(dernier, "capitaux_propres")
    flux, rn = val(dernier, "flux_exploitation"), val(dernier, "resultat_net")

    return {
        "exercices": len(annees), "suffisant": True,
        "de": annees[0], "a": dernier,
        "croissance_ca_pct": croissance("produit_exploitation"),
        "croissance_resultat_pct": croissance("resultat_net"),
        "croissance_capitaux_propres_pct": croissance("capitaux_propres"),
        "marges": marges, "marge_derniere": marges.get(dernier),
        "marge_tendance": (round(marges[dernier] - marges[annees[0]], 1)
                           if dernier in marges and annees[0] in marges else None),
        "roes": roes, "roe_dernier": roes.get(dernier),
        "exercices_deficitaires": pertes,
        "exercices_en_recul": reculs,
        # Un bénéfice qui ne se transforme pas en trésorerie est un bénéfice
        # comptable : c'est le premier signe de qualité des résultats.
        "conversion_tresorerie_pct": (round(flux / rn * 100) if flux and rn and rn > 0
                                      else None),
        "dette_sur_fonds_propres_pct": (round(dettes / cp * 100) if dettes and cp
                                        else None),
    }


# Les postes de solvabilité et de trésorerie rejoignent l'extraction standard.
POSTES.extend(POSTES_SUP)
...
def collecter(verifier_sika: bool = True, avec_dividendes: bool = True,
    ...
    if avec_etats:
        # Cache disque : un exercice publié ne change plus. Sans lui, chaque
        # collecte retélécharge deux cents PDF pour rien.
        cache_f = DOSSIER / "etats_cache.json"
        ...
        for u in univers:
            ...
            if not cible:
                continue
            exercices, docs = {}, tous_etats_financiers(session, cible)
            for doc in docs:
                if doc["url"] in cache:
                    e = cache[doc["url"]]
                else:
                    ...
                    if etat != "ok":
                        cache[doc["url"]] = {"etat": etat}
                        scannes += etat == "scanne"
                        continue
                    telecharges += 1
                    e = {"etat": "ok",
                         "exercice": exercice_du_document(doc["url"], texte),
                         "postes": extraire_etats(texte), "url": doc["url"],
            ...
            if not exercices:
                u["etats"] = {"etat": "aucun_exploitable",
                              "documents": len(docs)}
                continue
            recent = max(exercices)
            postes = exercices[recent]
            val = u.get("valorisation") or {}
            div = (u.get("dividendes") or {}).get("dernier_montant_net")
            u["etats"] = {
                "etat": "ok", "exercice": recent,
                "url": next((c["url"] for c in cache.values()
                             if isinstance(c, dict) and c.get("exercice") == recent
                             and c.get("etat") == "ok"), None),
                "postes": postes,
                "ratios": ratios_etats(postes, div, val.get("actions_estimees")),
                "serie": serie_fondamentale(exercices),
            }
            res_ok += 1
        cache_f.write_text(json.dumps(cache, ensure_ascii=False), encoding="utf-8")
        pluri = sum(1 for u in univers
                    if (u.get("etats") or {}).get("serie", {}).get("suffisant"))
...
def hebdomadaire_dates(lignes: list[dict], n: int = 260) -> str | None:
    """Date du premier point hebdomadaire retenu : l'application en a besoin
    pour dater l'axe des abscisses."""
    vu, dates = set(), []
    for s in lignes:
        ...
        if cle not in vu:
            vu.add(cle)
            dates.append(s["date"])
    return dates[-n:][0] if dates else None
...
def ecrire_app(univers: list[dict], histoires: dict, meta: dict) -> None:
    ...
    for u in univers:
        ...
        titres.append({
            ...
            "w": w,
            "w0": hebdomadaire_dates(histoires.get(u["ticker"], [])),
            "div": {
            ...
            "fin": {
                ...
                "url": ef.get("url"),
                "rn": ((ef.get("postes", {}).get("resultat_part_groupe") or {}).get("valeur")
                       or (ef.get("postes", {}).get("resultat_net") or {}).get("valeur")),
                ...
                "serie": ef.get("serie"),
                "exercice": ef.get("exercice"),
            } if ef else None,
            "ann": [{"d": a["date"], "t": a["type"], "i": a["important"],
                     "x": a["titre"][:140], "u": a["url"]} for a in ann[:8]] or None,
        })
    # Médianes sectorielles : un PER de 10 ne se juge que contre ses pairs.
    # Une banque et une agro-industrie n'ont pas la même structure de bilan.
    par_secteur: dict[str, dict[str, list]] = {}
    for t in titres:
        d = par_secteur.setdefault(t["s"], {"per": [], "payout": [], "roe": [], "marge": []})
        if t.get("val", {}) and t["val"].get("per"):
            d["per"].append(t["val"]["per"])
        f = t.get("fin") or {}
        for cle, v in (("payout", f.get("payout") or (t.get("val") or {}).get("payout")),
                       ("roe", f.get("roe")), ("marge", f.get("marge"))):
            if v is not None:
                d[cle].append(v)
    medianes = {
        sec: {k: (round(statistics.median(v), 1) if len(v) >= 3 else None)
              for k, v in d.items()}
        for sec, d in par_secteur.items()
    }
    for t in titres:
        t["sect"] = medianes.get(t["s"])
